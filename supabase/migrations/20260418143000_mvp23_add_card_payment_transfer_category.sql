...
-- 1) Add card payment transfer category to system catalog.
...
values
  ('transfer_card_payment', 'transfer', 'Pago de tarjeta', 'Card payment', null, 20, null, true)
...
-- 2) Ensure every workspace has a local category instance.
...
join public.system_categories sc
  on sc.key = 'transfer_card_payment'
where not exists (
  select 1
...
-- 3) Map common legacy aliases to the new semantic bucket.
